...
class pdf_waldeks_proposal extends ModelePDFPropales
{
	...
	public $description = 'Minimal clean PDF module for Waldeks Proposal';
	...
	{
		global $user, $langs, $conf, $mysoc, $db, $hookmanager, $nblines, $user, $object;
		...
		$pdf->SetFont('', 'B', 14);
		$pdf->SetXY($this->marge_gauche, $posy);
		$pdf->Cell(0, 8, $outputlangs->transnoentities("Angebot"), 0, 1, 'L');
		...
		// Draw rectangle box
		$box_height = $line_height * 4 + 4; // 6 rows + 2 padding
		$pdf->SetLineWidth(0.2);
		...
		//$pdf->Cell($col2_width, $line_height, "Angebot", 0, 0, 'R');
		...
		// Ihre Anfragenummer (custom field, use your logic here or fallback to ref_client)
		$pdf->SetXY($box_x + 2, $current_y);
		$pdf->Cell($col1_width, $line_height, "Ihre Anfragenummer:");
		...
		// Angebotsdatum in German date format: DD.MM.YYYY
		$pdf->SetXY($box_x + 2, $current_y);
		$pdf->Cell($col1_width, $line_height, "Angebotsdatum:");
		...
		$paragraph = "Wir danken für Ihre Anfrage, die wir zu unseren Verkaufs- und Lieferbedingungen (unter: www.waldeks.com/agb oder kostenfrei bei uns anzufordern) unverbindlich, auf Basis der jeweils aktuellen Metallnotierungen/Marktsituation, wie folgt anbieten.";
...
// $pdf->Line($this->marge_gauche, $posy + 1, $this->page_largeur - $this->marge_droite, $posy + 1);
		...
		// Lieferzeit
		$pdf->SetFont('', 'B', $default_font_size);
		$pdf->MultiCell($note_width, 5, "Lieferzeit:", 0, 'L');
		$pdf->SetFont('', '', $default_font_size);
		$lieferzeit = !empty($object->array_options['options_lieferzeit']) ? $object->array_options['options_lieferzeit'] : 'ca. 5-6 Wochen abgehend - Zwischenverkauf vorbehalten';
		$pdf->MultiCell($note_width, 5, $lieferzeit, 0, 'L');
		...
		$pdf->SetFont('', '', $default_font_size);
		$lieferbedingungen = !empty($object->array_options['options_lieferbedingungen']) ? $object->array_options['options_lieferbedingungen'] : "Die Preise verstehen sich ab Werk ausschl. Verpackung.\nDie Ware bleibt bis zur restlosen Bezahlung unser Eigentum.\nErfüllungsort und Gerichtsstand ist Rosenheim\n\nDieses Angebot ist 15 Tage gültig.";
		$pdf->MultiCell($note_width, 5, $lieferbedingungen, 0, 'L');
		...
		$pdf->SetFont('', '', $default_font_size);
		$zahlungsbedingungen = !empty($object->array_options['options_zahlungsbedingungen']) ? $object->array_options['options_zahlungsbedingungen'] : '14 Tage netto';
		$pdf->MultiCell($note_width, 5, $zahlungsbedingungen, 0, 'L');

		// Save
		$dir = $conf->propal->dir_output . "/" . dol_sanitizeFileName($object->ref);
		dol_mkdir($dir);
		$customName = "Waldeks - Angebot " . dol_sanitizeFileName($object->ref);
